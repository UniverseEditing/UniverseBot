        ...
        switch (interaction.options.getSubcommand()) {
            ...
            case "create":
                await DB.create({ GuildID: interaction.guild.id, ChannelID: "None", SuggestionManagers: [], AllowOwnSuggestionDelete: false, Disabled: true }).then(async () => {
                    await interaction.reply({ embeds: [new MessageEmbed().setColor("#ffffff").setTitle(`Suggestion system`).setDescription(`The suggestion system has successfully been created for ${interaction.guild.name}. \n\n To allow you to setup this system, the \`/suggest\` is currently disabled.`).setFooter({ text: "DM Jonas#1713 for help." })] })
            ...
            case "set-channel":
                ...
                try {
                    await channel.send({ embeds: [new MessageEmbed().setColor("#ffffff").setDescription(`> ✅ This channel has been set as a suggestions channel.`).setFooter({ text: "DM Jonas#1713 for help." })] }).then(async () => {
                        await DB.findOneAndUpdate({ GuildID: interaction.guild.id }, { ChannelID: channel.id }, { new: true, upsert: true })
                        interaction.reply({ embeds: [new MessageEmbed().setColor("#ffffff").setDescription(`> ✅ ${channel} has successfully been set as the suggestions channel for ${interaction.guild.name}.`)] })
                    })
                } catch (error) {
            ...
            case "reset":
                DB.deleteOne({ GuildID: interaction.guild.id })
                    .then(() => {
                        return interaction.reply({ embeds: [new MessageEmbed().setColor("#ffffff").setDescription(`> ✅ The suggestions channel has successfully been reset.`)] })
                    })
                break;
            case "enable":
                ...
                return interaction.reply({ embeds: [new MessageEmbed().setColor("#ffffff").setDescription(`> ✅ The suggestion system has been enabled.`)] })
            ...
            case "disable":
                ...
                return interaction.reply({ embeds: [new MessageEmbed().setColor("#ffffff").setDescription(`> ✅ The suggestion system has been disabled.`)] })
            ...
            case "suggestion-managers":
                switch (interaction.options.getString("option")) {
                    case "view":
                        const suggestionManagers = suggestSetup.SuggestionManagers.length <= 0 || !suggestSetup.SuggestionManagers ? "None" : `<@&${suggestSetup.SuggestionManagers.join(">, <@&")}>`
                        return interaction.reply({ embeds: [new MessageEmbed().setColor("#ffffff").setTitle(`Suggestion manangers for ${interaction.guild.name}`).setDescription(`Please note that these members can accept, decline and delete suggestions.\n\n${suggestionManagers}`)] })
                        break;
                    case "add":
                        ...
                        return interaction.reply({ embeds: [new MessageEmbed().setColor("#ffffff").setDescription(`> ✅ ${role} has been added as a suggestion manager.`)] })
                    ...
                    case "remove":
                        ...
                        return interaction.reply({ embeds: [new MessageEmbed().setColor("#ffffff").setDescription(`> ✅ ${role} has been removed as a suggestion manager.`)] })
            ...
            case "allow-own-suggestion-delete":
                ...
                if (allowOwnSuggestionDelete) {
                    ...
                    return interaction.reply({ embeds: [new MessageEmbed().setColor("#ffffff").setDescription(`> ✅ Members can now delete their own suggestions.`)] })
                } else {
                    await DB.findOneAndUpdate({ GuildID: interaction.guild.id }, { AllowOwnSuggestionDelete: false })
                    return interaction.reply({ embeds: [new MessageEmbed().setColor("#ffffff").setDescription(`> ✅ Members can now not delete their own suggestions.`)] })
                }
                break;
        }
